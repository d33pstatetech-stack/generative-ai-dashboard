...
// Coarse CivitAI baseModel → arch family string (feeds the pickers' loraFamily).
...
async function handleApiRoute(request, env, path, url) {
  ...
  {
    const m2 = path.match(/^\/api\/loras\/custom\/(\d+)$/);
    if (m2 && request.method === 'DELETE') {
      const hdb = histDB(env);
      if (!hdb) return jsonResponse({ error: 'history DB not bound' }, 500);
      await ensureCustomLoras(hdb);
      await hdb.prepare('DELETE FROM custom_loras WHERE id = ?').bind(Number(m2[1])).run();
      return jsonResponse({ ok: true, id: Number(m2[1]) });
    }
  }
